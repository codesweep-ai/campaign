...
| Renderer | `--viewer <bin>` / `DISPATCH_FIXTURES_VIEWER`; else `go build ./dispatch-viewer/cmd/cs-dispatch-viewer`. Every render runs with an empty PATH, so a `cs-tracer` on the host's PATH changes nothing the suite measures, and every page carries the `tracer-absent` warning. |
