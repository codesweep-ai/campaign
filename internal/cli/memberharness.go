...
func harnessRemedy(campaign string, sandboxVersion string) string {
	return strings.Join([]string{
		...
		"  image keeps stale tools for its entire life, and no reboot repairs it. The host",
		"  checks in `cs-campaign doctor` (the cs-sandbox pin, the agent tools) speak only",
		"  for the host.",
		"",
